...
def spinor_movie_maker(
    ...
    def animate_spectropolarimeter(i):
        ...
        try:
            ...
            u_s.set_array(uslit)
            v_s.set_array(vslit)
            inorm = Normalize(np.mean(islit) - 3*np.std(islit), np.mean(islit) + 3*np.std(islit))
            i_s.set_norm(inorm)
            qnorm = Normalize(np.mean(qslit) - 2*np.std(qslit), np.mean(qslit) + 2*np.std(qslit))
            q_s.set_norm(qnorm)
            unorm = Normalize(np.mean(uslit) - 2*np.std(uslit), np.mean(uslit) + 2*np.std(uslit))
            u_s.set_norm(unorm)
            vnorm = Normalize(np.mean(vslit) - 2*np.std(vslit), np.mean(vslit) + 2*np.std(vslit))
            v_s.set_norm(vnorm)
