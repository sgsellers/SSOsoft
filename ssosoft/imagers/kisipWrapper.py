...
class kisipWrapper:
    ...
    def kisip_configure_run(self):
        ...
        self.kisipArcsecPerPixX=self.plate_scale[0]
        self.kisipArcsecPerPixY=self.plate_scale[1]
    ...
    def kisip_spawn_kisip(self):
        ...
        self.logger.info("KISIP command: {0}".format(kisip_command))
        ...
        try:
            ...
            self.logger.error("Something went wrong with KISIP run. "
                    "Check logfile. Code: {0}".format(return_code)
                    )
            raise
        else:
            self.logger.info(
